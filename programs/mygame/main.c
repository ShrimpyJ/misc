...
#define KEY(key) keystate[SDL_SCANCODE_##key]
...
Player *player_init()
{
  ...
  p->pos = vec3_init(0, 0, 0);
  return p;
}

void move_left(Player *p)
{
  p->pos = vec3_sub(p->pos, (vec3){1, 0, 0});
}

void move_right(Player *p)
{
  p->pos = vec3_add(p->pos, (vec3){1, 0, 0});
}

void move_forward(Player *p)
{
  p->pos = vec3_add(p->pos, (vec3){0, 0, 1});
}

void move_backward(Player *p)
{
  p->pos = vec3_sub(p->pos, (vec3){0, 0, 1});
}

void jump(Player *p)
{
  p->pos = vec3_add(p->pos, (vec3){0, 1, 0});
}
...
int main(int argc, char **argv)
{
  ...
  Player *player = player_init(0, 0, 0);
  ...
  while(open){
    ...
    while (SDL_PollEvent(&e)){
      ...
      switch(e.type){
        ...
        case SDL_KEYDOWN:
          if (KEY(ESCAPE)) open = 0;

          /* Movement */
          if (KEY(W)) move_forward(player);
          if (KEY(S)) move_backward(player);
          if (KEY(A)) move_left(player);
          if (KEY(D)) move_right(player);
          if (KEY(SPACE)){
            SDL_TimerID timer = SDL_AddTimer(33, jump(player)
            jump(player);
          }
