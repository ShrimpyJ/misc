...
int has_same(char n[], char *word)
{
  int i, j;
  int max;
  for (i = 0; i < strlen(n); i++){
    for (j = 0; j < strlen(word); j++){
      if (n[i] == word[j]) return 1;
    }
  }
  return 0;
}

int findValid(int index, char n[], Pair *word, Pair **pairs, int npairs, Answer *answer)
{
  int i, j;
  int result;
...
//    result = n & pairs[i]->num;
    result = has_same(n, pairs[i]->word);
    char m[80];
    memset(m, 80, 0);
    strcpy(m, n);
    strcat(m, pairs[i]->word);
    if (result == 0){
      /* Add answer to answerlist and recurse with new answerlist */
      answer_push(answer, pairs[i]);
      for (j = i; j < npairs; j++){
        findValid(j+1, m, pairs[j], pairs, npairs, answer);
        break;
      }
      answer_pop(answer);
    }
...
//  free(words);
  ...
  for (i = 0; i < index; i++){
    ...
    char n[80];
    memset(n, 80, 0);
    strcpy(n, pairs[i]->word);

    findValid(i+1, n, pairs[i], pairs, index, answer);
    answer->nitems = 0;
    free(answer);
  }
  printf("Done\n");
