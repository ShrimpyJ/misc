...
void removePiece(vector<Queen> &qvec);
...
int main()
{
	...
	do{
		...
		q.space = space;
		bool is_totalTaken = isTaken(q.space, totalTaken);
		if (is_totalTaken == true){
			cout << "Illegal move.\n";
			continue;
		}
...
{
	int i;
	cout << "0. Clear all pieces\n";
	for (int j = 0; j < qvec.size(); j++){
		cout << j+1 << ". " << qvec[j].name << '\n';
	}
	cout << "Enter # of piece to remove: ";
	cin >> i;
	if (i == 0){
		qvec.clear();
	}else{
		qvec.erase(qvec.begin() + (i-1));
	}
}
