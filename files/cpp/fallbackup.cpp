...
public:
	...
	virtual void moveObject(vector<Object*> &v)
	{
	...
	virtual bool isThere(vector<Object*> v, int i, int j)
	{
...
void printScreen(WINDOW *win, Player player, vector<vector<Object*>> Objects, Object obj, int timer, int rpos, int lpos, int color)
{
	...
	for (int i = 0; i < COL; i++){
		for (int j = 0; j < ROWS+1; j++){
			bool is_enemy = obj.isThere(Objects[0],i,j);
			bool is_trail = obj.isThere(Objects[1],i,j);
			bool is_boost = obj.isThere(Objects[2],i,j);
			bool lboost = obj.isThere(Objects[2],i,j-1);
			bool rboost = obj.isThere(Objects[2],i,j+1);
			bool is_lwall = obj.isThere(Objects[3],i,j);
			bool is_rwall = obj.isThere(Objects[4],i,j);
			bool is_lsign = obj.isThere(Objects[5],i,j);
			bool is_rsign = obj.isThere(Objects[6],i,j);
			if (is_lsign == true || is_rsign == true){
				wattron(win, COLOR_PAIR(SIGN));
				waddch(win, 'I');
				wattroff(win, COLOR_PAIR(SIGN));
			}
			else if (is_lwall == true || is_rwall == true){
				wattron(win, COLOR_PAIR(WALL+color));
				waddch(win, 'I');
				wattroff(win, COLOR_PAIR(WALL+color));
			}
			...
			else if (is_trail == true){
				int r;
				if (timer % 2 == 0){r = TRAIL;}
				else{r = PLAYER;}
				wattron(win, COLOR_PAIR(r));
				waddch(win, '|');
				wattroff(win, COLOR_PAIR(r));
			}
			else if (is_enemy == true){
				wattron(win, COLOR_PAIR(ENEMY));
				waddch(win, 'U');
				wattroff(win, COLOR_PAIR(ENEMY));
			}
			else if (is_boost == true || lboost == true || rboost == true){
				wattron(win, COLOR_PAIR(BOOST));
				waddch(win, 'V');
				wattroff(win, COLOR_PAIR(BOOST));
			}
			else{
				waddch(win, ' ');
			}
...
int main()
{
	...
	init_color(COLOR_GREEN, 1000, 0, 0);
